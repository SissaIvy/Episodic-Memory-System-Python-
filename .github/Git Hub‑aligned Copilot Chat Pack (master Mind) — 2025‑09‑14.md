...
## MCP: Add GitHub remote server (official config)

**File**: `.vscode/mcp.json`  
**Purpose**: Enable the GitHub MCP server in this workspace.
...
### Settings additions (schema‑clean)
Merge the following keys into `.vscode/settings.json` to wire up instruction, prompt, and mode file discovery using the schema‑preferred *map* format:
...
> Optional: In Settings, search for **“MCP auto start”** and set **`chat.mcp.autoStart`** to your preference (default is `never`).

### Verify
1) **Restart VS Code** → open Chat → select **Agent mode** → click **Tools**. You should see tools from **github‑mcp**.  
2) If tools don’t appear, run **MCP: List Servers**, start **github‑mcp**, and accept the trust prompt.  
3) If stale, run **MCP: Reset Cached Tools** and retry.

### Security
- Do **not** hard‑code tokens in `mcp.json`. If the server requests credentials, VS Code will prompt; supply them via secure prompts or environment variables.

### Rollback
- Remove the `github-mcp` entry from `.vscode/mcp.json`.  
- Remove the added keys from `.vscode/settings.json`.  
- Restart VS Code.
...
## PR boilerplate (copy/paste)
**Title**: chore: align Copilot Chat with GitHub policy  
**Body**:
- Aligns VS Code Copilot Chat with GitHub’s customization model (instruction files, prompt files, chat modes, MCP wiring).
- Schema‑clean settings (map locations) and GitHub MCP server; no secrets committed.
- Rollback instructions included in the pack.

---

## Post‑merge verification checklist
1. Restart VS Code → Chat → select **Planning / Research / Test Author** modes; run `/plan-feature`, `/review-security`, `/generate-tests`.
2. Agent mode → **Tools** → start **github‑mcp**; complete interactive auth; confirm tools appear.
3. Settings: confirm no schema warnings for `chat.*FilesLocations`; prompt `"/"` shows your prompt files.

---

## Troubleshooting (quick)
- **MCP tools don’t appear**: Command Palette → *MCP: List Servers*; start **github‑mcp**; *MCP: Reset Cached Tools* and retry.
- **Schema squiggles**: Ensure locations are **objects** (e.g., `{ ".github/prompts": true }`), not arrays; reload window.
- **Network/auth issues**: Use a fine‑grained PAT limited to this repo; avoid corporate proxy blocking `https://api.githubcopilot.com`.

---

## Security & PAT scopes (minimum)
- Use a **fine‑grained** PAT scoped to **this repository**.
- Grant read access to: Repository contents (required). Add Pull Requests/Issues **read** only if your tools need them.
- Escalate to write scopes only when enabling tool actions that modify repo state.

---

## Line endings note (.gitattributes)
To prevent LF/CRLF churn across platforms, consider adding:

```gitattributes
* text=auto
```

(Adjust per team policy if you prefer consistent LF.)

---

## Ownership & maintenance
- **Maintainer(s)**: <add GitHub handles>
- **Review cadence**: quarterly; verify instructions and prompts reflect current practices.
- **Change control**: treat changes as `chore(tooling)`; include rollback notes in each PR.
