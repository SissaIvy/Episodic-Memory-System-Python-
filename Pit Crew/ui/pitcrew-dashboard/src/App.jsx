...
const DEFAULT_GATES = { fr_strict: 0.05, fr_explore: 0.05, xai_min: 0.95 };
...
const PitLanes = ({ data, width=820, height=260 }) => {
  ...
  return (
    <svg width={width} height={height} role="img" aria-label="Pit Lanes - strict vs explore" className="rounded-2xl shadow-md">
      ...
      <g transform={`translate(${width-30},${margin.t-10})`}>
        <circle cx={10} cy={10} r={9} fill={data.decision==="PASS"?PAL.pass:PAL.hold}/>
        <text x={10} y={11} fill="#fff" fontSize={12} textAnchor="middle">{data.decision==="PASS"?"✓":"!"}</text>
      </g>
      {cats.map((c)=>{
