...
def process_file(
    ...
    try:
        ...
        normalized = normalize(original)  # noqa: F821  (validated in main)
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    # Validate runtime dependency only after parsing so --help works.
    if normalize is None:  # pragma: no cover - runtime dependency check
        print(
            f"Failed to import 'terminology_normalizer.normalize': {_IMPORT_ERROR}",
            file=sys.stderr,
        )
        sys.exit(2)
